...
class TestSingleCourse:
    ...
    def test_single_course_lowercase(self):
        prereq, coreq, err = parse("ceic1000")
        assert err is None
        assert prereq == "CEIC1000"
...
class TestAndOrExpressions:
    ...
    def test_ampersand_treated_as_and(self):
        prereq, coreq, err = parse("CEIC1000 & CEIC1001")
        assert err is None
        assert prereq == {"and": ["CEIC1000", "CEIC1001"]}

    def test_comma_treated_as_and(self):
        prereq, coreq, err = parse("CEIC1000, CEIC1001")
        assert err is None
        assert prereq == {"and": ["CEIC1000", "CEIC1001"]}
...
class TestInvalidInputs:
    def test_unrecognized_token_returns_error(self):
